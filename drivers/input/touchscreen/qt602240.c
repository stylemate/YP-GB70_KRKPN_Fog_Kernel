...
#ifdef CONFIG_MACH_ARIES//defined(CONFIG_MACH_S5PC110_PALLADIO)
unsigned char test_node[TEST_POINT_NUM] = {14,              22,              109,             196,           204};
#else
...
#define CONFIG_GFF_PANEL
#ifdef CONFIG_GFF_PANEL
#define	GFF_PANEL	1
#define	G1F_PANEL	0
#define	INV_PANEL	0xff
static int gPanelType = 0;
#endif

#define FREESPACE_TUNE
#ifdef FREESPACE_TUNE
#define TCHTHR_VAL	35
#else
#define TCHTHR_VAL	40
#endif

#define ANTITOUCH_TUNE
#ifdef ANTITOUCH_TUNE
#define TCHDRIFT_VAL	20
#define ATCHCALST_VAL	10
#define ATCHCALSTHR_VAL	7
#else
#define TCHDRIFT_VAL	5
#define ATCHCALST_VAL	9
#define ATCHCALSTHR_VAL	35
#endif
...
void qt_Acquisition_Config_Init(void)
{
	if (info_block->info_id.family_id == 0x80) {
		...
		acquisition_config.tchdrift = TCHDRIFT_VAL;
...
void qt_Multitouchscreen_Init(void)
{
	if (info_block->info_id.family_id == 0x80) {
		...
		touchscreen_config.tchthr = TCHTHR_VAL;
		touchscreen_config.tchdi = 2;
		touchscreen_config.orient = 1;
//#if (defined(CONFIG_MACH_S5PC110_PALLADIO)&& defined(CONFIG_GFF_PANEL))
#if (defined(CONFIG_MACH_ARIES)&& defined(CONFIG_GFF_PANEL))
		if (gPanelType == GFF_PANEL) {
			printk(" *** GFF Panel *** (orient)\n");
			touchscreen_config.orient = 3;
		}
#endif	// CONFIG_GFF_PANEL
...
#endif
#ifdef _SUPPORT_MULTITOUCH_	
		touchscreen_config_e.numtouch= MAX_USING_FINGER_NUM;// it is 5 now and it can be set up to 10
#else
...
void qt_Multitouchscreen_stylus_Init(void)
{
	if (info_block->info_id.family_id == 0x80) {
		...
		touchscreen_config.movfilter = 79;
		touchscreen_config.numtouch= 1;
		...
		touchscreen_config_e.tchthr = 25;
		touchscreen_config_e.movhysti = 1;

		touchscreen_config_e.movfilter = 79;
		touchscreen_config_e.numtouch= 1;
...
#if defined(CONFIG_GFF_PANEL)
	if (gPanelType == GFF_PANEL) {
		printk(" *** GFF Panel *** (xorigin)\n");
...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	if (info_block->info_id.family_id == 0x80) {
		...
		noise_suppression_config.freq[1] = 35;
		noise_suppression_config.freq[2] = 42;
		noise_suppression_config.freq[3] = 50;
		noise_suppression_config.freq[4] = 57;
...
static int get_panel_type(void)
{
    ...
    uint16_t keyarray_addr=0;
	uint8_t xorigin = 0;

    keyarray_addr = get_object_address(TOUCH_KEYARRAY_T15, 0);

#if 0
	if (read_mem(keyarray_addr, 1, &xorigin) == 1u)
		printk(" *** keyarray.ctrl= %d\n", xorigin);
	if (read_mem(keyarray_addr+1, 1, &xorigin) == 1u)
		printk(" *** keyarray.xorigin= %d\n", xorigin);
	if (read_mem(keyarray_addr+2, 1, &xorigin) == 1u)
		printk(" *** keyarray.yorigin= %d\n", xorigin);
	if (read_mem(keyarray_addr+3, 1, &xorigin) == 1u)
		printk(" *** keyarray.xsize= %d\n", xorigin);
	if (read_mem(keyarray_addr+4, 1, &xorigin) == 1u)
		printk(" *** keyarray.ysize= %d\n", xorigin);
#endif

	status = read_mem(keyarray_addr+1, 1, &xorigin);  // xorigin = keyarray_addr + 1
	if (status != READ_MEM_OK) {
		printk(" *** read_mem failure ***\n");
		return status;
	} else {
		printk(" *** keyarray.xorigin= %d\n", xorigin);
	}

	if (xorigin == 0) {	// GFF PANEL
		gPanelType = GFF_PANEL;
		printk(" *** GFF Panel *** \n");
	} else if (xorigin == 15) {	// G1F Panel : 15
		gPanelType = G1F_PANEL;
		printk(" *** G1F Panel *** \n");
	} else {
		gPanelType = INV_PANEL;
		printk(" *** Invaild Panel *** \n");
		return 0;	// Invalid panel
	}
#if 0
	while(1) {
		msleep(100);
...
#if defined(CONFIG_GFF_PANEL)
	if (get_panel_type() != READ_MEM_OK) return; /* if there are error to get panel info, skip the reg. settings */
#endif
	...
	if ((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00)) {
		...
		* page byte will sit at 1 until we next send F3 command */
		...
		if ((tch_ch > 0) && (atch_ch == 0)) {  //jwlee change.
			...
			dprintk("[TSP] calibration was bad\n");
			calibrate_chip();
			qt_timer_state = 0;
			qt_time_point = jiffies_to_msecs(jiffies);
		} else {
...
void  get_message(void)
{
	...
	if ((quantum_msg[0] < 2) || (quantum_msg[0] >= 12)) {
		...
		if (quantum_msg[0] == 1) {
		...
		}else if (quantum_msg[0] == 12) { /* Touch Key */
			...
			if (quantum_msg[1] == 0x80) { 
				...
				if (quantum_msg[2] == 1) {
					...
					input_sync(qt602240->input_dev);
					keycode = KEY_MENU;//TOUCHKEY_KEYCODE_MENU;
					...
					input_sync(qt602240->input_dev);
					keycode = KEY_BACK;//TOUCHKEY_KEYCODE_BACK;
				...
				if (keycode != 0) {
					...
					if (checkTSPKEYdebuglevel != KERNEL_SEC_DEBUG_LEVEL_LOW)
						printk("touchkey released(%d) : (%d)\n",quantum_msg[2], keycode);
					keycode = 0;
				}
			} else {
				/* error */
				printk(KERN_ERR"[%s] touchkey error \n", __FUNCTION__);
			}
		} else if (quantum_msg[0] == 14) {  /* Palm Touch */
			if ((quantum_msg[1]&0x01) == 0x01) {
				dprintk(KERN_DEBUG"[TSP] Palm Touch press\n");
				touch_message_flag = 1;
				one_touch_input_flag = 1; //hugh 0312
				TSP_forced_release();
			} else
				dprintk(KERN_DEBUG"[TSP] Palm Touch relese\n");
		...
		if ((quantum_msg[0] ==  12) || (quantum_msg[0] ==  14) || (quantum_msg[0] == 0) || (quantum_msg[0] == 0xFF)) {
...
if (checkTSPKEYdebuglevel != KERNEL_SEC_DEBUG_LEVEL_LOW)
	...
	if (press == 1) {
		...
		printk("[sehun]%s x=%3d, y=%3d, BTN=%d, size=%d, amp=%d\n",__FUNCTION__, x, y,press, size , amplitude);
	} else if (press == 0) {
	    input_sync(qt602240->input_dev);
	    amplitude = quantum_msg[6];
	    printk("[sehun]%s x=%3d, y=%3d, BTN=%d, size=%d, amp=%d\n",__FUNCTION__, x, y,press, size , amplitude);
...
#ifdef CONFIG_MACH_ARIES//CONFIG_MACH_S5PC110_PALLADIO
	keyled_onoff(1);
#endif
